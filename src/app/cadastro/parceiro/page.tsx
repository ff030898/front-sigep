...
import {
  ...
  Modal, ModalOverlay, ModalContent, ModalHeader, ModalFooter, ModalBody, ModalCloseButton, useDisclosure, Input, Link as ChakraLink, Image, Select, Spacer
...
const CadastroParceiro: React.FC = () => {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      const response = await axios.post("http://[::1]:3001/parceiros", data, {});
      setModalTitle('Sucesso');
      setModalMessage(`Cadastro realizado com sucesso! ID do parceiro: ${response.data.id}`);
      onOpen();
    } catch (error: unknown) {
  ...
  return (
    <ChakraProvider>
      <Box display="flex" flexDirection={{ base: "column", md: "row" }} width="100vw" height="100vh" overflow="hidden">
        <Box
          display="flex"
          ...
          height="100vh"
        >
          <VStack spacing={4} align="stretch" width="100%" maxWidth="500px" as="form" onSubmit={handleSubmit(onSubmit)} flex="1">
            ...
            <FormControl isInvalid={!!errors.cliente}>
              <FormLabel>Qual o nome do seu cliente?</FormLabel>
              <Select placeholder="Selecione" {...register("cliente")}>
                {mockClientes.map(cliente => (
                  <option key={cliente.id} value={cliente.nome}>{cliente.nome}</option>
                ))}
              </Select>
              <Text color="red.500">{errors.cliente?.message}</Text>
            </FormControl>
            ...
            <Button colorScheme="orange" type="submit" width="100%" height="50px">
              Criar Conta
            </Button>
        ...
        >
          <Image
            src="/imagens/imagemParceiroCadastro.png"
            alt="Imagem de Cadastro de Parceiro"
            objectFit="cover"
            width="100%"
            height="100%"
          />
